/*
 * @Author: M_King 
 * @Date: 2017-08-29 23:11:22 
 * @Last Modified by: M_King
 * @Last Modified time: 2017-08-29 23:14:37
 */
...
function tipShow(parm) {
    ...
    */
    ...
    var notClose = parm[notClose] || true; //初始化是否关闭
...
function tipStart() {
    //设置弹窗层
    var temp = '<div class="popup_wrap"><div class="popup_bg"></div><div class="popup_content"><div><div class="popup_box"></div></div></div></div>';
    //输出弹窗
    $("body").append(temp);
}
...
function verification(form, error, success, post) {
    ...
    var name = ["is_name", "is_qq", "is_tell", "is_mobile", "is_id", "is_email", "is_pass", "is_bank", "is_num", "is_ch", "is_empty"];
    //正则表达式数组
    var regex = [/[^a-zA-Z0-9]/, /^\d{5,11}$/, /^0\d{2,3}-{0,1}\d{7,8}$|^\d{7,8}$/, /((1[34578][0-9]{1}))\d{8}/, /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/, /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/, /\d+[a-zA-Z]+|[a-zA-Z]+\d+/, /^\d{19}$|^\d{12}$|^\d{16}$/, /[^0-9]/, /[\u4E00-\u9FFF]/, /[\u4E00-\u9FFF]/];
    //反向数组
    var rev = [0, 8];
    //获取要验证的对象
    var obj = $("[ver]");
